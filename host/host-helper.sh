...
audit() {
  ...
  printf '{"ok":true,"os":"%s","kernel":"%s","architecture":"%s","active_provider":"%s","storage":{"data_mounted":%s,"tailscale_state":%s,"netbird_state":%s},"tailscale":{"installed":%s,"healthy":%s,"version":"%s"},"netbird":{"installed":%s,"healthy":%s,"version":"%s"}}\n' \
   "$(printf %s "$os"|json_escape)" "$kernel" "$arch" "$active" "$(mountpoint -q /DATA && echo true || echo false)" "$(test -s /DATA/AppData/tailscale/tailscaled.state && echo true || echo false)" "$(test -d /DATA/AppData/netbird && find /DATA/AppData/netbird -type f -print -quit|grep -q . && echo true || echo false)" \
...
case "$ACTION" in
 audit) audit;;
 install|update) [[ "$PROVIDER" == tailscale ]] && install_tailscale || install_netbird; audit;;
 repair) if [[ "$PROVIDER" == tailscale ]]; then test -x /usr/bin/tailscale || install_tailscale; systemctl restart tailscaled; else test -x /usr/bin/netbird || install_netbird; systemctl restart netbird; fi; audit;;
 restart) systemctl restart "$([[ "$PROVIDER" == tailscale ]] && echo tailscaled || echo netbird)"; audit;;
 up) do_up;;
 down) [[ "$PROVIDER" == tailscale ]] && /usr/bin/tailscale down || /usr/bin/netbird down; audit;;
 rollback) test -s "$DATA/rollback/$PROVIDER.raw"; cp -f "$DATA/rollback/$PROVIDER.raw" "/var/lib/extensions/$PROVIDER.raw"; systemd-sysext refresh; systemctl daemon-reload; systemctl restart "$([[ "$PROVIDER" == tailscale ]] && echo tailscaled || echo netbird)"; audit;;
 *) exit 64;;
esac
